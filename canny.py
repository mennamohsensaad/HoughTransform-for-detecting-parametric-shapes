...
def hough_line(image):
    Ny = image.shape[0]
    Nx = image.shape[1] 
    Maxdist = int(np.round(np.sqrt(Nx**2 + Ny ** 2))) #p
    thetas = np.deg2rad(np.arange(-90, 90))
    rs = np.linspace(-Maxdist, Maxdist, 2*Maxdist)
    accumulator = np.zeros((2 * Maxdist, len(thetas))) # acumalator array  initialized by zeros 
    for y in range(Ny):        #for each pixel (x,y)
        for x in range(Nx):
            if image[y,x] > 0:
                 for k in range(len(thetas)):   #to counter acuumalator theta 
                    r = x*np.cos(thetas[k]) + y * np.sin(thetas[k]) #calculate  
                    accumulator[int(r) + Maxdist,k] += 1   #increment uumalator r,theta 
